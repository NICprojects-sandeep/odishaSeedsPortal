...
@Component({
  selector: 'app-pfmsfarmersale',
  templateUrl: './pfmsfarmersale.component.html',
  styleUrls: ['./pfmsfarmersale.component.css']
})
export class PfmsfarmersaleComponent implements OnInit {
  title: string;
    ...
    this.service.InsertSaleDealer(alldata).subscribe(data => {
      ...
      if (data.result == "True") {
        ...
        if(this.subsidyModeToERUPI == 1){
          this.toastr.success(`The e-Rupi voucher is generated once the qrcode or token is send to the farmer's mobile phone, 
          and the shared qrcode or token is reedem from the farmer's mobile.`);
