...
public partial class Farmer_FarmerList : System.Web.UI.Page
{
    ...
    {
        gvFarmerFamilyDtls.DataSource = ds12.Tables[0];
        gvFarmerFamilyDtls.DataBind();
    }
 
    protected void ImgbtnShowDtls_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ImageButton btnShow = sender as ImageButton;
            GridViewRow row = (GridViewRow)btnShow.NamingContainer;

            HiddenField hfFARMERID = (HiddenField)row.FindControl("hfFARMERID");
            HiddenField hfFARMERCODE = (HiddenField)row.FindControl("hfFARMERCODE");

            DataSet ds12 = new DataSet();
            string[] param = { "@farmercode" };
            object[] value = { hfFARMERCODE.Value };
            ds12 = dbsFarmer.param_passing_proc("SP_FamilyMemberDtls", param, value);
            if (ds12 != null)
            {
