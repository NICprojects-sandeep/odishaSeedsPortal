...
public partial class Reports_RPT_Schemewisedtl1 : System.Web.UI.Page
{
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        string[] param = { "@SCHEME_CODESP", "@FIN_YEAR", "@SEASON", "@CROP_ID", "@VCHDISTID" };
        object[] value = { drpScheme.SelectedValue.Trim(), drpSession.SelectedValue.ToString(), drpSeason.SelectedValue.ToString() == "0" ? System.Convert.DBNull : drpSeason.SelectedValue.ToString(), drpCropName.SelectedValue.Trim() == "--- All ---" ? System.Convert.DBNull : drpCropName.SelectedValue.Trim(), drpDistrict.SelectedValue.Trim() == "--- All ---" ? System.Convert.DBNull : drpDistrict.SelectedValue.Trim() };
    ...
    private void GenerateReport(DataSet ds)
    {
        ...
        foreach (DataRow dr in dist.Rows)
        {
            ...
            if (drpDealer.SelectedIndex > 0)
            {
                filter = "DealerPacks='" + drpDealer.SelectedValue.Trim() + "' ";//" + drpDealer.SelectedValue.Trim();

            }
            if (filter != string.Empty)
            {
                filter = filter + " and NDist_Code='" + dr["NDist_Code"].ToString() + "'";
            }
            else
            {
