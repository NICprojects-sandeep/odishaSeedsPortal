...
public partial class Reports_OSSC_Farmer_Incentive_OilSeed : System.Web.UI.Page
{
    BLL_DropDown BAL;
    DLL_DropDown DAL;
    DataSet ds;
    ...
    private void FillGrid()
    {
        ...
        //ds = DAL.GetIncentiveOilSeed(BAL);
        //ProcessToServer(ds);

        ds = DAL.FillIncentiveOilSeed(BAL);
        GenerateReport(ds);
    }
    ...
    private void ProcessToServer(DataSet ds)
    {
        ...
        if (dt1 != null)
        {
            if (dt1.Rows.Count > 0)
            {
                ...
                DAL = new DLL_DropDown();
                DAL.InsertIncentiveOilSeed(BAL, out Val);
                if (Val == 1)
                {
