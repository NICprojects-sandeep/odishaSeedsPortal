...
public partial class Reports_OSSC_Farmer_Incentive_OilSeedAll :  System.Web.UI.Page
{
    BLL_DropDown BAL;
    DLL_DropDown DAL;
    DataSet ds;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            //    int Cnt = DAL.FillPendingIncentiveOilSeedAll(BAL);
            //    if (Cnt > 0)
            //    {
            //        dvOSSC.Visible = true;
            //    }
            ...
            //    int Cnt = DAL.FillPendingIncentiveOilSeedDeptAll(BAL);
            //    if (Cnt > 0)
            //    {
            //        dvDEPT.Visible = true;
            //    }
    ...
    private void FillGrid()
    {
        BAL = new BLL_DropDown();
        BAL.FIN_YR = lblFinYr.Text;
        BAL.SEASON = ddlSeason.SelectedValue;
        ds = new DataSet();
        DAL = new DLL_DropDown();

        //ds = DAL.GetIncentiveOilSeedAll(BAL);
        //ProcessToServer(ds);

        ds = DAL.FillIncentiveOilSeedAll(BAL);
        GenerateReport(ds);
    }
    private void GenerateReport(DataSet ds)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            str.Append("<td style='text-align: Right;'>OSSC Status</td>");
    ...
    private void ProcessToServer(DataSet ds)
    {
        ...
        if (dt1 != null)
        {
            if (dt1.Rows.Count > 0)
            {
                ...
                DAL = new DLL_DropDown();
                DAL.InsertIncentiveOilSeedAll(BAL, out Val);
                if (Val == 1)
                {
    ...
    protected void btnPay_Click(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(txtTrn.Text))
        {
            ...
            BAL.FIN_YR = lblFinYr.Text;
            DAL = new DLL_DropDown();
            DAL.OsscAprvOilSeedDeptAll(BAL);
            dvDEPT.Visible = false;
            FillGrid();
        }
        else
