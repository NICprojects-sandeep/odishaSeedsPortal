...
public partial class Reports_OSSC_Farmer_Incentive_Pulse : System.Web.UI.Page
{
    BLL_DropDown BAL;
    DLL_DropDown DAL;
    DataSet ds;
    ...
    private void FillGrid()
    {
        ...
        ////BEGIN
        //ds = DAL.GetIncentivePulse(BAL);
        //ProcessToServer(ds);
        ////END

        //BEGIN
        ds = DAL.FillIncentivePulse(BAL);
        if (ds != null)
        {
    ...
    private void GenerateReport(DataSet ds)
    {
        ...
        str.Append("<table style='width:1325px; border-collapse:collapse; font-size: 14px; ' border='1' cellpadding='5' cellspacing='0' class='tabletxt'>");
        str.Append("<tr style='background-color:#CCCCCC; font-weight:bold;'>");
        str.Append("<td style='text-align: Left;color: blue'>Agency : </td>");
        str.Append("<td style='text-align: Left;color: black'>OSSC </td>");
        str.Append("<td style='text-align: Left;color: blue'>Crop : </td>");
        str.Append("<td style='text-align: Left;color: black'>Pulse </td>");
        str.Append("<td style='text-align: Left;color: blue'>Financial Year : </td>");
        str.Append("<td style='text-align: Left;color: black'>" + ddlFinYear.SelectedValue + " </td>");
        str.Append("<td style='text-align: Left;color: blue'> Variety Type : </td>");
        str.Append("<td style='text-align: Left;color: black'>Less than 10 Years </td>");
        str.Append("</tr>");
        str.Append("</table>");
    ...
    private void ProcessToServer(DataSet ds)
    {
        ...
        if (dt1 != null)
        {
            if (dt1.Rows.Count > 0)
            {
                ...
                DAL = new DLL_DropDown();
                DAL.InsertIncentivePulse(BAL, out Val);
                if (Val == 1)
                {
