...
public partial class CCBpayment : System.Web.UI.Page
{
    ...
    private void FillCCBRequest()
    {
        try
        {
            ...
            objUserDLL = new DLL_DropDown();
            ds = new DataSet();
            ds = objUserDLL.FillCCBRequest(objUserBEL);

            if (ds != null)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    gridRequest.DataSource = ds;
                    gridRequest.DataBind();
                }
            }
        }
    ...
    protected void gridRequest_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Select")
        {
            ...
            GridViewRow row = gridRequest.Rows[rowIndex];
    ...
    private void Updatedtls(string CPAID)
    {
        try
        {
            objUserBEL = new BLL_DropDown();
    ...
    private void FillDtls(string CPAID)
    {
        try
        {
            objUserBEL = new BLL_DropDown();
